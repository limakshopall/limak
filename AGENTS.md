...
- **Commande** : `/commande` + `app/commande/actions.ts` (`createOrder` : recalcule les prix côté serveur, **transaction anti-survente** qui vérifie+décrémente le stock, rattache `clerkUserId` si connecté, envoie **SMS de confirmation au client** + **email d'alerte à l'admin** (`ADMIN_EMAIL`, via Resend — remplace l'ancienne alerte SMS pour réduire les coûts)), `/commande/merci`. Commande **invité** possible.
...
- **SMS** `app/lib/sms.ts` (appel REST via `fetch`, formatage numéro CI `+225`) : confirmation client, suivi de statut (`AT_USERNAME`/`AT_API_KEY` dans `.env` — à vérifier aussi dans Vercel, sinon échec silencieux en prod). L'alerte admin est passée en **email** (voir `app/lib/email.ts`, `sendAdminOrderAlertEmail`, `ADMIN_EMAIL` dans `.env`). Un envoi SMS/email ne bloque JAMAIS une commande/action.
